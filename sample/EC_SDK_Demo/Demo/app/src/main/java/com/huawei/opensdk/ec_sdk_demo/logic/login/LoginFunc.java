        ...
        switch (msg.what)
        {
            case LOGIN_SUCCESS:
                ...
                Executors.newSingleThreadExecutor().execute(new Runnable()
                {
                    ...
                    public void run()
                    {
                        mSeqNo = EnterpriseAddressBookMgr.getInstance().searchSelfInfo(LoginCenter.getInstance().getAccount());
                    }
                });
                break;
            case LOGIN_FAILED:
    ...
    public void onReceive(String broadcastName, Object obj) {
        switch (broadcastName)
        {
            case CustomBroadcastConstants.ACTION_ENTERPRISE_GET_SELF_RESULT:
                List<TupEaddrContactorInfo> selfInfo = (List<TupEaddrContactorInfo>) obj;
                TupEaddrContactorInfo contactorInfo = selfInfo.get(0);
                String terminal = contactorInfo.getTerminal();
                LoginCenter.getInstance().setSelfInfo(contactorInfo);
                LoginCenter.getInstance().getSipAccountInfo().setTerminal(terminal);
                LogUtil.i(UIConstants.DEMO_TAG, "terminal-->" + terminal);
                break;
            default:
                break;
        }
