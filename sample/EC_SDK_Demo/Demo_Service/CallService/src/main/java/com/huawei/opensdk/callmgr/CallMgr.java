...
public class CallMgr extends TupCallBackBaseNotify implements ICallMgr
{
    ...
    public synchronized int accessReservedConf(String confId, String accessCode, String password, boolean isVideo)
    {
        ...
        if (LoginCenter.CLOUD_PBX == solution)
        {
    ...
    {
        //设置ipt权限值
        IptMgr.getInstance().updateRegisterStatus(list);
    }
    ...
    {
        IIptNotification iptNotify = IptMgr.getInstance().getIptNotification();
        if (iptNotify != null)
        {
            iptNotify.onSetIptServiceSuc(serviceCallType);
        }
    }
    ...
    public void onSetIptServiceFal(int serviceCallType)
    {
        ...
        if (iptNotify != null)
        {
            iptNotify.onSetIptServiceFal(serviceCallType);
        }
    }
